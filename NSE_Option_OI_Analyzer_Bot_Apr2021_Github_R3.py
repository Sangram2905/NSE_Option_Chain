...
class NseOI:
    ...
    def graphsAll(self,event=None):
        ...
        ## IV Graph
        time_data = self.time_count
        civ_data = self.civ_list
        piv_data = self.piv_list
        iv_data = {'Time':time_data,'Call_IV':civ_data,'Put_IV':piv_data}
    ...
    def main_win(self):
        ...
        self.options.add_separator()
        # self.options.add_command(label="Live Graph Start/Stop", command=self.toggle) ## Curently under construction 
        # self.options.add_separator()
    ...
    def main(self):
        ...
        if len(self.civ_list) <= 0:
            ...
            self.civ_list = [(sum(self.civ_list)/len(self.civ_list))]
        ...
        if len(self.piv_list) <= 0:
            ...
            self.piv_list = [(sum(self.piv_list)/len(self.piv_list))]
        

        ## OI Logic
        OI_dfC = df.iloc[:, 0]
        self.cOI_list = []
        for numi in range(10):
        ...
        self.totaOI = sum(self.cOI_list) + sum(self.pOI_list)
