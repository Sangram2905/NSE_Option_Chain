...
class NseOI:
    ...
    def graphsAll(self,event=None):
        ...
        # OI Graph
        OI_PLOT = df_sd.plot(x=0,y=9,label = "OI" , ax=axes[1,0])
        
        # OI difference graph
        OI_Diff_PLOT = df_sd.plot(x=0,y=7,label = "Max3_CHOI_Dif",ax=axes[0,1])
        
        # CH_OI PCR difference graph
        CH_OI_PCR_PLOT = df_sd.plot(x=0,y=10,label = "CH_OI_PCR",ax=axes[2,0])
        
        
        # 'Put OI','Call OI' graph
        OI_PLOT = df_sd.plot(x=0,y=[6,5],label = ['M3Put CHOI','M3Call CHOI'],ax=axes[1,2] )
        
        # Volume difference graph
        Vol_PLOT = df_sd.plot(x=0,y=8,label = "Vol_diff",ax=axes[0,2])
    ...
    def main_win(self):
        ...
        # oi_ub_label = Label(bottom_frame, text="Max_OI", relief=RIDGE,
        #                     font=("TkDefaultFont", 10, "bold"))
        # oi_ub_label.grid(row=0, column=0, sticky=N + S + W + E)
        

        
        current_Time_label = Label(bottom_frame, text="Sys_Update_Time:", relief=RIDGE,
                                     font=("TkDefaultFont", 10, "bold"))
        current_Time_label.grid(row=0, column=0, sticky=N + S + W + E)
            
        self.current_time_val = Label(bottom_frame, text="", relief=RIDGE)
        self.current_time_val.grid(row=0, column=1, sticky=N + S + W + E)
        
        diff_Time_label = Label(bottom_frame, text="Time_Diff:", relief=RIDGE,
                                     font=("TkDefaultFont", 10, "bold"))
        diff_Time_label.grid(row=0, column=2, sticky=N + S + W + E)
        
        self.diff_Time_val = Label(bottom_frame, text="", relief=RIDGE,font=("TkDefaultFont", 10, "bold"))
        self.diff_Time_val.grid(row=0, column=3, sticky=N + S + W + E)
    
        
        max_call_oi_sp_label = Label(bottom_frame, text="Max St P:", relief=RIDGE,
                                     font=("TkDefaultFont", 10, "bold"))
        ...
        oi_lb_label = Label(bottom_frame, text="Min_OI", relief=RIDGE,
                            font=("TkDefaultFont", 10, "bold"))
        oi_lb_label.grid(row=0, column=4, columnspan=4, sticky=N + S + W + E)
        
        max_put_oi_sp_label = Label(bottom_frame, text="Min St P:", relief=RIDGE, font=("TkDefaultFont", 10, "bold"))
        ...
        pcr_label = Label(bottom_frame, text="CH_OI_PCR:", relief=RIDGE, font=("TkDefaultFont", 10, "bold"))
    ...
    def set_values(self):
        ...
        # # Logic for live graph
        # if self.first_run is not True:
        #     plt.close()
        # sheet_data = self.sheet.get_sheet_data()
        # df_sd = pandas.DataFrame(sheet_data)
        # #print (df_sd)
        # CHOI_PLOT = df_sd.plot(x=0,y=[4,7],label = ["CH_OI_Diff","M3CHOI"])
        # plt.xlabel('Time')
        # plt.ylabel('CH_OI_Diff')
        # plt.title({self.underlying_stock}) 
        # plt.legend()
        # plt.show()
    ...
    def main(self):
        ...
        try:
            self.put_itm = round((total_call_vol - total_put_vol)/1000, 1) #volume difference logic
        except ZeroDivisionError:
            self.put_itm = 0
        ...
        df_vol= df['Open Interest'].astype(float)
        df_Call_vol = df_vol.iloc[:,0]
        df_Put_vol = df_vol.iloc[:,1]
